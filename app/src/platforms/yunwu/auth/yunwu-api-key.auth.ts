...
/**
 * 云雾平台 API Key 认证策略
 * id 只写简短形式 'api-key'，注册中心自动拼装为 'yunwu.api-key'
 */
export class YunwuApiKeyAuthStrategy extends BaseAuthStrategy {
  getMetadata(): AuthStrategyMetadata {
    return {
      id: 'api-key',
      name: 'API Key 认证',
      description: '使用云雾平台 API Key 进行认证',
      authSchema: {
        description: '请输入您的云雾平台 API Key 和 Base URL',
        fields: [
          {
            name: 'apiKey',
            type: 'string',
            required: true,
            description: '云雾平台 API Key',
            uiHint: 'text',
          },
          {
            name: 'baseUrl',
            type: 'string',
            required: false,
            description: 'API 基础 URL（可选，默认: https://yunwu.ai/v1）',
