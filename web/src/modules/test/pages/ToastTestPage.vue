  ...
  <TestLayout title="Toast 通知" description="测试所有 Toast 通知类型的展示效果与动画" badge="UI" badge-color="blue">
    <TestCard title="触发测试" subtitle="点击按钮触发对应类型的 Toast 通知">
      <div class="flex flex-wrap gap-1">
        <button
          class="flex h-8 items-center justify-center gap-1.5 rounded-md px-2 text-xs text-gray-500 transition-all hover:bg-gray-100 hover:text-emerald-600"
          @click="toastStore.show('操作已成功完成，数据已保存。', 'success')"
        >
          <CheckCircleOutlined class="h-3.5 w-3.5" />
          成功
        </button>
        <button
          class="flex h-8 items-center justify-center gap-1.5 rounded-md px-2 text-xs text-gray-500 transition-all hover:bg-gray-100 hover:text-red-600"
          @click="toastStore.show('请求失败，请检查网络连接后重试。', 'error')"
        >
          <CancelOutlined class="h-3.5 w-3.5" />
          错误
        </button>
        <button
          class="flex h-8 items-center justify-center gap-1.5 rounded-md px-2 text-xs text-gray-500 transition-all hover:bg-gray-100 hover:text-amber-600"
          @click="toastStore.show('此操作不可逆，请谨慎操作。', 'warning')"
        >
          <WarningAmberFilled class="h-3.5 w-3.5" />
          警告
        </button>
        <button
          class="flex h-8 items-center justify-center gap-1.5 rounded-md px-2 text-xs text-gray-500 transition-all hover:bg-gray-100 hover:text-blue-600"
          @click="toastStore.show('任务正在后台处理，稍后完成。', 'info')"
        >
          <InfoOutlined class="h-3.5 w-3.5" />
          提示
        </button>
      </div>
    </TestCard>
    ...
    <TestCard title="批量堆叠测试" subtitle="同时触发多条通知，测试堆叠效果">
      <button
        class="flex h-8 items-center justify-center gap-1.5 rounded-md px-2 text-xs text-gray-500 transition-all hover:bg-gray-100 hover:text-gray-700"
        @click="triggerAll"
      >
        触发全部类型
      </button>
